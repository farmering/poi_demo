...
//					System.out.println("+cellValue+"+cellValue);
					...
					//把数据插入模板表INSPECTION_OPINION_TEMPLATE
	 ...
	 * 保存检验项目对应的意见书
	 */
	public void saveIORows(String inspectTypeId) {
		Vector vec=jianYanBO.getList("SELECT *　FROM INSPECTION_OPINION_TEMPLATE WHERE INSPECT_TYPE_ID='"+inspectTypeId+"' AND FIELD_CELLTYPE='button'");
		if(vec!=null&&vec.size()>0) {
			List<String>list=new ArrayList<String>();
			for (Object object : vec) {
				Hashtable hashtable=(Hashtable) object;
				String fieldDec=(String) hashtable.get("FIELD_COMMENT");
				String fieldId=(String) hashtable.get("FIELD_ID");
				String fieldName=(String) hashtable.get("FIELD_NAME");
				list.add(fieldName);
				if("".equals(fieldDec)) {
					continue;
				}
				Vector vecOld=jianYanBO.getList("SELECT *　FROM INSPECTION_OPINION_DEC WHERE INSPECT_TYPE_ID='"+inspectTypeId+"' AND FIELD_NAME='"+fieldName+"'");
				if(vecOld!=null&&vecOld.size()>0) {
					jianYanBO.UpdSql("DELETE FROM INSPECTION_OPINION_DEC  WHERE INSPECT_TYPE_ID='"+inspectTypeId+"' AND FIELD_NAME='"+fieldName+"'");
				}
				if(fieldDec.contains("@@@")) {
					String[]arrs=fieldDec.split("@@@");
					for (String str : arrs) {
						Hashtable hashtableNew=new Hashtable();
						String newid=UUID.randomUUID().toString().replaceAll("-", "");
						hashtableNew.put("ID",newid);//主键id
						hashtableNew.put("FIELD_ID",fieldId );//单元格id
						hashtableNew.put("FIELD_NAME",fieldName );//意见描述对应的单元格名称
						hashtableNew.put("FIELD_DEC",str );//意见描述
						hashtableNew.put("INSPECT_TYPE_ID",inspectTypeId );//检验报告模板id，如000301001对应：定期检验，杂物电梯定期检验报告
						jianYanBO.InsertData(hashtableNew, "INSPECTION_OPINION_DEC");
					}	
				}else {
					Hashtable hashtableNew=new Hashtable();
					String newid=UUID.randomUUID().toString().replaceAll("-", "");
					...
					hashtableNew.put("FIELD_ID",fieldId );//单元格id
					hashtableNew.put("FIELD_NAME",fieldName );//意见描述对应的单元格名称
					hashtableNew.put("FIELD_DEC",fieldDec );//意见描述
					hashtableNew.put("INSPECT_TYPE_ID",inspectTypeId );//检验报告模板id，如000301001对应：定期检验，杂物电梯定期检验报告
					jianYanBO.InsertData(hashtableNew, "INSPECTION_OPINION_DEC");
				}
			}
			//意见书中的检验项目在模板记录表中没有此检验项目，则删除此意见书
			Vector vecOld=jianYanBO.getList("SELECT *　FROM INSPECTION_OPINION_DEC WHERE INSPECT_TYPE_ID='"+inspectTypeId+"'");
			if(vecOld!=null&&vecOld.size()>0) {
				for (Object object : vecOld) {
					Hashtable hashtable=(Hashtable) object;
					String id=(String) hashtable.get("ID");
					String fieldName=(String) hashtable.get("FIELD_NAME");
					if(!list.contains(fieldName)) {
						jianYanBO.delReport("DELETE  FROM INSPECTION_OPINION_DEC WHERE ID='"+id+"'");
					}
				}
			}
		}
